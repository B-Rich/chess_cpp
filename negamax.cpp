...
/*int zero_search(Chessboard brd, int depth){
    int rating = MIN_INT
    if(depth == MAX_DEPTH)
    return  evaluate(brd,depth)
}*/
/**typedef struct{
    int depth;
    int alpha;
...
Node ab_search(int alpha, int beta, int depth, Chessboard board, Moves move) {
    ...
    if (leaf_node || no_available_moves) {
        ...
        data.depth = depth;
        data.move = move;
        data.rating = evaluate(board, depth, possible_moves.length()/4);
        return data;
        //return max_score;
    }
    ...
    for (int m = 0; m < possible_moves.length() / 4; m++) {
        ...
        if (dest != NULL) move_piece(mimic, dest, cur_move, promo_unit);
        player = !player;
        data = ab_search(alpha, beta, depth - 1, board, cur_move);
        mimic = board;
        if(player == white){
           if(data.rating > alpha){
               alpha = data.rating;
           }
        }else{
            if(data.rating <= beta){
...
Moves albe_search(int alpha, int beta, int depth, Chessboard board, Moves move) {
    ...
    for (int m = 0; m < possible_moves.length() / 4; m++) {
        ...
        move_piece(mimic, src, cur_move, promo_unit);
        if (dest != NULL) move_piece(mimic, dest, cur_move, promo_unit);
        Moves best = albe_search(alpha, beta, depth - 1, board, cur_move);
        ...
        if(depth == 2) move = best.substr(0,4);
    }
    return move;
}